...
class MazeState(AbstractState):
    ...
    def get_neighbors(self, ispart1=False):
        ...
        for nbr in neighboring_locs:
            new_nbr = MazeState(nbr, self.goal, dist_from_start=self.dist_from_start + 1, maze=self.maze, mst_cache=self.mst_cache, use_heuristic=self.use_heuristic)
            nbr_states.append(new_nbr)

        return nbr_states

    # TODO: implement this method
    def is_goal(self):
        # self.goal: (x-coordinate, y-coordinate, radius)
        # self.state: (row, col, level)
    ...
    def compute_heuristic(self):
        shortest_manh = manhattan(self.state, self.goal[0])
        for i in range(len(self.goal)-1):
            curr_manh = manhattan(self.state, self.goal[i+1])
            if (curr_manh < shortest_manh):
                shortest_manh = curr_manh

        return shortest_manh
