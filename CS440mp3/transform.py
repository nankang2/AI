...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    """
    map_width = int(window[0] / granularity) + 1
    map_height = int(window[1] / granularity) + 1
    new_map = [[ [SPACE_CHAR for shape in range(3)] for col in range(map_height)] for row in range(map_width)]
    #print(new_map)
    #set start point
    config = alien.get_config()
    idx = configToIdx(config, (0, 0, 0), granularity, alien)
    
    for i in range(map_width):
        for j in range(map_height):
            for k in range(3):
                config = idxToConfig((i, j, k), (0, 0, 0), granularity, alien) #position in real map
                alien.set_alien_config(config)
                if not is_alien_within_window(alien, window, granularity):
                    new_map[i][j][k] = WALL_CHAR
                
                elif does_alien_touch_wall(alien, walls, granularity):
                    new_map[i][j][k] = WALL_CHAR

                elif does_alien_touch_goal(alien, goals):
                    new_map[i][j][k] = OBJECTIVE_CHAR
                
    new_map[idx[0]][idx[1]][idx[2]] = START_CHAR
    new_maze = Maze(new_map, alien, granularity=granularity, offsets=[0, 0, 0], filepath=None,
                 use_heuristic=True)
